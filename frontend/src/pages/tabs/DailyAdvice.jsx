...
export default function DailyAdvice({ userName }) {
  ...
  return (
    <motion.div
      ...
      exit={{ opacity: 0, y: -20 }}
      className="px-6 pt-14 pb-6 min-h-[calc(100vh-80px)] flex flex-col"
    >
      <motion.div
        ...
        className="mb-2"
      >
        <h1 className="text-3xl font-bold text-persona-dark mb-1">Daily Advice</h1>
        <p className="text-persona-muted">Your personalized tip for today</p>
      </motion.div>
      ...
      <motion.div
        ...
        className="flex items-center gap-2 text-sm text-persona-muted mb-8"
      >
      ...
      <div className="flex-1 flex items-center justify-center">
        <AnimatePresence mode="wait" custom={direction}>
          <motion.div
            ...
            className={`w-full bg-gradient-to-br ${currentTip.color} rounded-3xl p-8 relative overflow-hidden`}
          >
            {/* Decorative */}
            ...
            <div className="relative">
              ...
              <div className="flex items-center gap-2 mb-6">
                ...
                <span className="text-sm font-semibold text-persona-dark/60 bg-white/50 px-3 py-1 rounded-full">
                  {currentTip.category}
                </span>
              </div>
              ...
              <div className="mb-6">
                ...
                <p className="text-xl font-medium text-persona-dark leading-relaxed">
                  "{currentTip.text}"
                </p>
              </div>
              ...
              <p className="text-sm text-persona-muted">
                Personalized for <span className="font-semibold text-persona-dark">{userName || 'Alex'}</span> · INTJ
              </p>
            </div>
          </motion.div>
        </AnimatePresence>
        ...
        {tips.map((_, i) => (
          <div
            ...
            className={`h-1.5 rounded-full transition-all duration-300 ${
              i === tipIndex ? 'bg-persona-dark w-6' : 'bg-gray-300 w-1.5'
            }`}
          />
        ))}
      </div>
